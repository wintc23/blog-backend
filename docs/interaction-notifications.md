...
邮件标题、发件人显示名和页脚统一使用数据库 `PersonalProfile.site_name`；数据库未填写时依次回退到 `SITE_NAME` 配置和通用称呼“本站”。发件邮箱地址仍取 `MAIL_SENDER`／`MAIL_USERNAME`，不修改 SMTP 账号。旧 `MAIL_SUBJECT_PREFIX` 不再作为站名来源。验证码和原有回复邮件也遵守这一规则；链接继续使用站点 URL 配置。
