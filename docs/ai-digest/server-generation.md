...
状态：服务端已部署，更新于 2026-09-18。本文为执行设计，实际实现、部署命令和限制见 [运行说明](operations.md)。scheduler / worker、后台配置与内容管理已完成；CPA 复用服务器 Codex 的 API 凭据和模型服务，已验证 GPT Image 2.5 Flare 生成及七牛上传。每日 08:30 自动生成、09:00 自动发布已启用。事实核对失败会在配置的重试次数和执行窗口内带着核对意见重新生成正文，通过核对后才可发布；复用来源快照和已生成的配图。
