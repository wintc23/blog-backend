...
状态：服务端已部署，更新于 2026-09-17。本文为执行设计，实际实现、部署命令和限制见 [运行说明](operations.md)。scheduler / worker、后台配置与内容管理已完成；CPA 复用服务器 Codex 的 API 凭据和模型服务，已验证 GPT Image 2.5 Flare 生成及七牛上传。每日 08:30 自动生成草稿已启用，自动发布关闭。已发布内容和图片保持原样。
