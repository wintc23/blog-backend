...
- **事实核对未通过**：按后台配置的重试次数和截止时间，自动带着核对问题及上轮文稿重新生成正文，再独立核对。资料快照、已生成和上传的配图复用；`checkpoint.review_history` 保留每轮失败的文稿与核对结果。手动重试也会清除失败的正文与核对缓存，不会原样重复失败。核对通过前不保存可发布修订；超过重试次数或执行窗口后保留失败状态供处理。已发布内容的重生成仍只创建候选修订，不自动覆盖线上版本。
