...
- `/products` 探索页展示已发布工具；`/tools` 兼容跳转探索页。
...
每张上传默认 20 MB、4000 万像素，后台可分别配置为 1–100 MB、1–80 百万像素；接受静态 JPEG/PNG/WebP，校正 EXIF 方向。原文件私有留存；交给模型的是按后台最长边设置等比缩小、剥离元数据的 PNG 副本（默认最长边 2048，可配置 512–4096；不放大小图，编码后最多 20 MB），结果也以 PNG 下载。图库公开上传接口不复用于这些私人照片。

所有原图、生成结果与中间配图均存七牛，不允许磁盘存储后备。浏览器持短期、固定 key、限大小和 MIME、禁止覆盖的凭证直传七牛；后端验证上传凭证归属并在内存解码，清除 EXIF 后把处理副本写入七牛。图片读取接口鉴权后重定向到七牛 HTTPS S3 签名 URL（有效期 60 秒），图片流量不经过网站服务器。分享封面同样重定向。批量下载接口只返回十分钟有效的云端文件清单，浏览器直接下载并在本机打 ZIP，不在服务器打包或中转下载。

配置 `IMAGE_TOOLS_QINIU_BUCKET`（专用私有空间）、`IMAGE_TOOLS_QINIU_REGION`（七牛 S3 区域）、`IMAGE_TOOLS_QINIU_UPLOAD_URL`（对应区域 HTTPS 上传地址），沿用七牛 AK/SK。部署时必须验证空间为私有、匿名读取被拒绝，配置 GET/HEAD 的 CORS。CORS 不代替签名授权。域名来自七牛区域配置，与网站部署域名无关。公开工具封面在原有公开空间 `image-tool-covers/`，仓库只留来源说明。

原图和结果默认保留。删除任务或撤销分享后网站停止签发访问链接；已经签发的七牛 URL 最多还有 60 秒有效期。批量下载清单为十分钟。七天未修改草稿和已删除任务由每日 cleanup 清理；一天以上且没有数据库引用的直传对象与写入中断对象一并回收，删除失败保留记录等待重试。清理不能删除尚在执行或限频窗口内的任务。

上传时只创建临时草稿，URL 保持工具页；只有点击生成并成功入队后才进入我的图片任务、后台任务记录，任务时间为首次提交时间。纯浏览不写草稿。

### 数据打点

复用站内 StatEvent，后台事件统计可查看 `image_tool.*`。前端记录 `view`、`upload_start`、`upload_failed`、`generate_click`、`submit_failed`、`download_complete`；后端事务内记录 `upload_complete`、`submitted`、`retry`、`result`、`download`、`download_all`。提交事件按一次批量计数并附图片数，执行结果按每张计数、含状态和耗时；重试独立计数，重复提交及轮询不重复记账。仅记录工具标识、数量、状态、来源和耗时，不含图片、文件名、提示词或签名 URL。
...
工具前台默认只展示输入和结果，进入工具即显示工作台，首次上传/手机上传/生成时自动创建任务。补充提示词、可选画幅和画风放在默认折叠的高级设置中，后端只接受模板允许范围内的参数。数量由后台固定配置（default_count）控制，逐张处理始终是一张原图对应一张结果（并非强制正方形）。逐张处理模式每张输入对应一张输出，文字生图使用后台设定的数量。任务继续保存创建时的配置快照。后台提供封面上传、可视化配置、发布状态、限频设置和任务记录。默认封面使用七牛公开空间 image-tool-covers/ 下的 WebP 资源，不依赖部署域名。
...
配置私有七牛空间后，停止旧图片 worker，分别执行以下命令；每个对象上传后按 SHA-256 校验，通过后才删除对应源文件：

```bash
venv/bin/python image_tools.py migrate-storage --source instance/image-tools --delete-source
venv/bin/python image_tools.py migrate-storage --source var/content-assets --kind generation --delete-source
```

正式切换前先省略 `--delete-source` 验证，再完成发布及访问检查，最后执行带删除选项的命令。本地与线上使用同一数据库时需迁移两端历史目录。常规运行代码不再读取磁盘图片；CLI 生图配图适配器读取工具输出后立即清除产物，AI 聊天的 CLI 输入使用调用期间的临时目录（Linux 使用内存 tmpfs），调用结束清除，不作为图片存储。


### 上传自动优化

客户端上传前读取 `/api/image-tools/upload-policy/`，先检查原图大小、像素和最长边。范围内的文件原样直传七牛；超过时在浏览器内按比例调整、先尝试较高质量编码，再逐步缩小到符合要求。照片使用 JPEG，带透明度图片优先 WebP，不支持 WebP 编码时按实际返回的 PNG MIME 上传，保留透明背景。图片处理资源及时释放，批量逐张执行；损坏、无法解码或动态图片仍给出具体错误。

原图在用户设备不被修改；范围内保留的原图和处理副本独立存入七牛，服务器不落盘。上传授权保存当时的策略快照，因此管理员在上传途中修改设置不会导致已授权上传无故失败。服务端仍验证解码尺寸和文件字节，不能通过绕过浏览器突破上限。前台提示优化过程；`image_tool.upload_compressed` 仅记录压缩前后 KB 数。

后台「生图管理 → 额度与图片设置」统一管理生成限频、上传大小、上传像素、生成输入最长边。限频规则保持不变。数据库升级：`venv/bin/python main.py db upgrade`。
